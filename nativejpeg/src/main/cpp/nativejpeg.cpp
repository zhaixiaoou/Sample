...
int compressJPEG(BYTE *data, int width, int height, jint quality, const char *path){
    ...
    struct native_error_mgr native_err;
    struct jpeg_compress_struct jcs;

    jcs.err = jpeg_std_error(&native_err.pub);

    if (setjmp(native_err.setjmp_buffer)){
        return 0;
    }
    ...
    jcs.arith_code = false; // 哈夫曼编码
    jcs.input_components = mComponent;
    jcs.in_color_space = JCS_RGB565;
    jpeg_set_defaults(&jcs);
    jcs.optimize_coding = true; // 优化
...
Java_com_example_nativejpeg_nativeCompressJPEG(JNIEnv *env, jclass type, jobject bitmap,
    ...
    // 一行一行的处理， 每个像素占4字节 包括a、r、g、b三个分量，每个分量占8位
